...
class TrackingAnything():
    ...
    def generator_image(self, image: np.ndarray, template_mask:np.ndarray, is_first:bool=False):
        ...
        # for i in tqdm(range(len(images)), desc="Tracking image"):
        if is_first:           
            mask, logit, painted_image = self.xmem.track(image, template_mask)
        else:
            mask, logit, painted_image = self.xmem.track(image)
